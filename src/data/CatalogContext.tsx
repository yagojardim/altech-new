...
export interface PriorityDef{ id:string; name:string; color:string; icon:string; order:number }
export interface IssueTypeDef{ id:string; name:string; icon:string; color:string; isDefault?:boolean }
export interface ComponentDef{ id:string; name:string; lead:string; desc:string }
export interface WorkflowStatus{ id:string; name:string; color:string; category:'todo'|'in-progress'|'done'; order:number }
export interface BoardColDef { id:string; name:string; statusIds:string[]; wipLimit:number|null }
export interface MemberDef   { id:string; name:string; initials:string; email:string; role:'Admin'|'Member'|'Viewer'; avatarColor:string }
...
const DEFAULT_CATALOG: Catalog = {
  ...
  issueTypes: [
    { id:'t1', name:'História',  icon:'◇', color:'#7d92ff', isDefault:true },
  ...
  workflow: [
    ...
    { id:'w3', name:'Em andamento', color:'#7d92ff', category:'in-progress', order:2 },
    { id:'w4', name:'Em revisão',   color:'#e6b23c', category:'in-progress', order:3 },
    { id:'w5', name:'Concluído',    color:'#35c9ae', category:'done',        order:4 },
  ],
  boardCols: [
  ...
  members: [
    { id:'m1', name:'Ana Lima',       initials:'AL', email:'ana@altech.io',    role:'Admin',  avatarColor:'#7d92ff' },
    { id:'m2', name:'Nuno Matos',     initials:'NM', email:'nuno@altech.io',   role:'Member', avatarColor:'#a78bfa' },
    { id:'m3', name:'João Neves',     initials:'JN', email:'joao@altech.io',   role:'Member', avatarColor:'#e6b23c' },
    { id:'m4', name:'Carla Silva',    initials:'CS', email:'carla@altech.io',  role:'Member', avatarColor:'#35c9ae' },
    { id:'m5', name:'Rui Melo',       initials:'RM', email:'rui@altech.io',    role:'Viewer', avatarColor:'#f0805c' },
    { id:'m6', name:'Lucas Ferreira', initials:'LF', email:'lucas@altech.io',  role:'Member', avatarColor:'#f97316' },
  ],
}
...
export function CatalogProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <CatalogContext.Provider value={{ catalog, setCatalog, updateLabels, updatePriorities, updateIssueTypes, updateComponents, updateWorkflow, updateBoardCols, updateMembers }}>
      {children}
